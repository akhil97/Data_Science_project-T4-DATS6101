...
Using exhaustive search we found that the variables age,balance,active_member predict the churn of customer which is a 3 variable best model with adjusted R^2 value 0.18.

The  best model predicted using BIC is age,balance,active_member which is a 3 variable model with BIC value -1900.

The best model predicted using Cp is credit_score,age,tenure,balance,active_member,estimated_salary which is a 6 variable model. with Cp value 4.2
...
The best model using forward search is age,balance,active_member which is a 3 variable model with adjusted R^2 value of 0.18.

The  best model predicted using BIC is age,balance,active_member which is a 3 variable model with value -1900 Cp.

The best model predicted using Cp is credit_score,age,tenure,balance,active_member_estimated_salary which is a 6 variable model with Cp value 4.2
...
The best model using backward search is age,balance,active_member which is a 3 variable model with adjusted R^2 value of 0.18.
...
The best model using Adj R^2 is country,gender,age,balance,active_member which is a 5 variable model with adjusted R^2 value of 0.14.
...
```{r, Model1}
churnglm1 <- glm(churn ~  gender + age + balance +active_member, data = churn_data_logit)
...
```{r, Model2}
churnglm2 <- glm(churn ~ country+ gender + age + balance + active_member, data = churn_data_logit)
```{r confusionMatrix_FS1, results='markup'}
loadPkg("regclass")
xkabledply( confusion_matrix(churnglm1), title = "Confusion matrix from Logit Model" )
```

```{r roc_auc_FS1, results='markup'}
loadPkg("pROC") 
prob=predict(churnglm1, type = "response" )
churn_data_logit$prob=prob
h <- roc(churn~prob, data = churn_data_logit)
auc(h) 
plot(h)
...
```{r results='markup'}
churnglm2 <- glm(churn ~ credit_score+age+tenure+ balance + active_member+estimated_salary, data = churn_data_logit)
summary(churnglm2)
...
```{r, Model3}
churnglm3 <- glm(churn ~ credit_score+country+gender+age+balance+active_member, data = churn_data_logit)
```{r confusionMatrix_FS2, results='markup'}
loadPkg("regclass")
xkabledply( confusion_matrix(churnglm2), title = "Confusion matrix from Logit Model" )
```

```{r roc_auc_FS2, results='markup'}
loadPkg("pROC") 
prob=predict(churnglm2, type = "response" )
churn_data_logit$prob=prob
h <- roc(churn~prob, data = churn_data_logit)
auc(h) 
plot(h)
...
```{r results='markup'}
churnglm3 <- glm(churn ~ credit_score + country+ gender+age+tenure+balance+active_member+credit_card+estimated_salary, data = churn_data_logit)
summary(churnglm3)
...
```{r, Model4}
churnglm4 <- glm(churn ~ credit_score + country+ gender+age+tenure+balance+active_member
```{r confusionMatrix_FS3, results='markup'}
loadPkg("regclass")
xkabledply( confusion_matrix(churnglm3), title = "Confusion matrix from Logit Model" )
```

```{r roc_auc_FS3, results='markup'}
loadPkg("pROC") 
prob=predict(churnglm3, type = "response" )
churn_data_logit$prob=prob
h <- roc(churn~prob, data = churn_data_logit)
auc(h) 
plot(h)
...
```{r results='markup'}
churnglm4 <- glm(churn ~ credit_score + country+ gender+age+tenure+balance+active_member+estimated_salary
...
```{r, Model5}
churnglm5 <- glm(churn ~ credit_score+country+gender+age+tenure+balance+active_member+estimated_salary, data = churn_data_logit)
```{r confusionMatrix_FS4, results='markup'}
loadPkg("regclass")
xkabledply( confusion_matrix(churnglm4), title = "Confusion matrix from Logit Model" )
```

```{r roc_auc_FS4, results='markup'}
loadPkg("pROC") 
prob=predict(churnglm4, type = "response" )
churn_data_logit$prob=prob
h <- roc(churn~prob, data = churn_data_logit)
auc(h) 
plot(h)
...
```{r results='markup'}
churnglm5 <- glm(churn ~ credit_score+country+gender+age+tenure+balance+estimated_salary, data = churn_data_logit)
summary(churnglm5)
...
```{r, Model6}
churnglm6 <- glm(churn ~ credit_score+country+gender+age+tenure+balance+products_number +active_member+estimated_salary, data = churn_data_logit)
```{r confusionMatrix_FS5, results='markup'}
loadPkg("regclass")
xkabledply( confusion_matrix(churnglm5), title = "Confusion matrix from Logit Model" )
```

```{r roc_auc_FS5, results='markup'}
loadPkg("pROC") 
prob=predict(churnglm5, type = "response" )
churn_data_logit$prob=prob
h <- roc(churn~prob, data = churn_data_logit)
auc(h) 
plot(h)
...
```{r results='markup'}
churnglm6 <- glm(churn ~ credit_score+gender+age+tenure+balance+estimated_salary, data = churn_data_logit)
summary(churnglm6)
...
```{r confusionMatrix_FS6, results='markup'}
loadPkg("regclass")
xkabledply( confusion_matrix(churnglm6), title = "Confusion matrix from Logit Model" )
```

```{r roc_auc_FS6, results='markup'}
loadPkg("pROC") 
prob=predict(churnglm6, type = "response" )
churn_data_logit$prob=prob
h <- roc(churn~prob, data = churn_data_logit)
auc(h) 
plot(h)
