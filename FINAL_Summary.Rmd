...
To perform dimenstionality reduction while preserving as much as randomness in the high dimensional space as possible. 
...
*45.Credit_score, country, gender, age, tenure, balance, estimated_salary(7 variable model)**
...
First, let us look at our base model, which includes all variables. We find in the summary that the credit_score, age, and balance variables are most significant (lowest p-values, as indicated by \*\*\*), with estimated_salary with slightly higher p-value (\*\*),, and tenure with a higher, but still significant p-value (\*).

**Model 1 - churn \~ age + balance**
The features for the first model is selected based on the higher vale of Adjusted R square, and lower values of BIC and Cp metrics using the forward search method. Looking at the Adjusted R square results we can see that we get a maximum value of 0.14 with minimum of 2 variables namely age and balance. Similarly, for BIC we get a minimum value of -1500 with 3 variables namely credit_score, age and balance and for minimum value of 7 for Cp from variables credit_score, age, tenure, balance, estimated_salary. From all the 3 parameters (Adjusted R square, BIC, Cp), we can see that age and balance variables are common, so we create the model using these features.
...
Model 1 was selected by four different feature selection methods, forward, backward, exhaustive, and sequential AdjR\^2, with the highest adjR\^2 of 0.14. Taking a look at the confusion matrix, we see that there are 2026 false negatives, with an area under the curve of 0.744. Let us see if we can improve on these numbers.
...
Model 2 was selected by four different feature selection methods: forward, backward, exhaustive, and sequential BIC, as having the lowest BIC. Age and balance are once again significant variables, whereas credit_card is not. Upon analysis of the confusion matrix, we find that this model has a false negative count of 1996, and an area under the curve of 0.744. This model is an improvement on 1 due to lower false negatives.
...
Model 3 was selected by four different feature selection methods: forward, backward, exhaustive, and sequential Cp, as having the lowest Cp (7). All variables included are significant, except active_member. Upon analysis of the confusion matrix, we find that this model has a false negative count of 2011, higher than Model 2. The area under the curve is 0.744, same as Models 1 & 2. Thus far, Model 2 is best.
...
Model 4 was selected by one feature selection method, Mallow CPG. Of the nine variables included, only five were significant: credit_score, age, tenure, balance, estimated_salary. Upon analysis of the confusion matrix, we find that this model has a false negative count of 2000 (better than model 3, not as good as Model 2). The area under the curve of 0.744, same as Models 1 & 2.
...
Model 5 was selected by one feature selection method, Mallow CPG. Of the eight variables included, only five were significant: credit_score, age, tenure, balance, estimated salary. Upon analysis of the confusion matrix, we find that this model has a false negative count of 1994, the lowest of the models we have seen so far, and an area under the curve of 0.745, same as Model 4. So far, Model 5 is the best model.
...
Model 6 was selected by one feature selection method: Mallow CPG. The same variables which were significant in Model 5 are significant in this model. Upon analysis of the confusion matrix, we find that this model has a false negative count of 2002, which is higher than the count in Model 5. The area under the curve is 0.745, same as Models 4 & 5. Model 5 is still the best model.
...
Finally, Model seven was selected by two different feature selection methods: Mallow CPG, and AdjR. All variables are significant, with the exception of gender. Upon analysis of the confusion matrix, we find that this model has a false negative count of 2001 and an area under the curve of 0.745, same as Models 5, and 6. Due to the higher count of false negatives, Model 7 fails to beat out Model 5 as the best model.
