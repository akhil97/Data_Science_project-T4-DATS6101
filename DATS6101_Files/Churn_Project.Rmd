...
## Determine whether age,country,gender predict the customer churn

## Uisng Linear Regression
```{r}
churn_model.final <- lm(churn~age+gender+country,data=churn_data_pc_target)
...
churn_age_fit <- lm(churn~ age, data = churn_data_pc_target)
...
churn_gender_fit <- lm(churn~ age+gender, data = churn_data_pc_target)
...
```{r model1,results='markup'}
churn_country_fit <- lm(churn~ age+gender+country, data = churn_data_pc_target)
...
churn_data_pc_target$gender <- factor(churn_data_pc_target$gender)
churn_data_pc_target$country <- factor(churn_data_pc_target$country)
...
churnLogitHoslem = hoslem.test(churn_data_pc_target$churn, fitted(churnLogit)) 
unloadPkg("ResourceSelection") 
churnLogitHoslem
```

```{r McFadden, results='markup'}
churnLogitNULL <- glm(churn ~ 1, data = churn_data_pc_target, family = "binomial")
mcFadden = 1 - logLik(churnLogit)/logLik(churnLogitNULL)
mcFadden
```

## Determine whether products,account status predict the customer churn

## Using Logistic regression
```{r results='markup'}
churn_data_pc_target$products_number <- factor(churn_data_pc_target$products_number)

churnLogit1 <- glm(churn ~ products_number+active_member, data = churn_data_pc_target, family = "binomial")
summary(churnLogit1)
...
```{r HosmerLemeshow,results='markup', collapse=F}
loadPkg("ResourceSelection") 
churnLogitHoslem = hoslem.test(churn_data_pc_target$churn, fitted(churnLogit1)) 
unloadPkg("ResourceSelection") 
churnLogitHoslem
```


## Feature selection uisng Exhaustive search  

```{r}
loadPkg("leaps")

reg.churn <- regsubsets(churn~.-customer_id, data = churn_data_pc_target, nvmax =9, nbest = 2, method = "exhaustive")  
...
## Feature selection uisng forward search  
```{r}
reg.churnforward <- regsubsets(churn~.-customer_id, data = churn_data_pc, nvmax =9, nbest = 2, method = "forward")
...
## Feature selection uisng Sequential Replacement
```{r}
...
## Feature selection uisng backward search  
```{r}
