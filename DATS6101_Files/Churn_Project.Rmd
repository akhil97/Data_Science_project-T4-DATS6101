---
title: "AMultinational Bank Customer Churn"
...
Balance:Maximum account balance of the cutsomer is 250898 with average balance being 76486.
...
churn_data_logit <- churn_data_pc_target[-c(1)]
churn_data_pc <- churn_data_logit[-c(11)]
str(churn_data_pc)
...
### Does the combination of variables such as country, age group, gender influence the churn rate?

```{r logitmodel,results='markup'}

churnLogit <- glm(churn ~ age + gender + country, data = churn_data_pc_target, family = "binomial")
summary(churnLogit)
...
```{r HosmerLemeshow, results='markup', collapse=F}
loadPkg("ResourceSelection") 
admitLogitHoslem = hoslem.test(churn_data_pc_target$churn, fitted(churnLogit)) 
...
```{r roc_auc}
loadPkg("pROC")
prob=predict(churnLogit, type = "response" )
churn_data_pc_target$prob=prob
h <- roc(churn~prob, data=churn_data_pc_target)
...
churnLogitpr2 = pR2(churnLogit)
churnLogitpr2
unloadPkg("pscl") 
```

### Customers using fewer products along with the status of the account affect the churn rate?

```{r logitmodel_SM2,results='markup'}
churnLogit_SM2 <- glm(churn ~ products_number + active_member, data = churn_data_pc_target, family = "binomial")
summary(churnLogit_SM2)

xkabledply(churnLogit_SM2, title = paste("Logistic Regression :", format(formula(churnLogit_SM2)) ))
```

```{r ConfInt_SM2, results='markup', collapse=F}
xkabledply( confint(churnLogit_SM2), title = "CIs using profiled log-likelihood" )

xkabledply( confint.default(churnLogit_SM2), title = "CIs using standard errors" )
```

```{r confusionMatrix_SM2, results='markup'}
loadPkg("regclass")

xkabledply( confusion_matrix(churnLogit_SM2), title = "Confusion matrix from Logit Model" )
unloadPkg("regclass")
```

```{r HosmerLemeshow_SM2, results='markup', collapse=F}
loadPkg("ResourceSelection") 
admitLogitHoslem_SM2 = hoslem.test(churn_data_pc_target$churn, fitted(churnLogit_SM2)) 
unloadPkg("ResourceSelection")
admitLogitHoslem_SM2
```

```{r roc_auc_SM2}
loadPkg("pROC")
prob_SM2 = predict(churnLogit_SM2, type = "response" )
churn_data_pc_target$prob_SM2 = prob_SM2
h_SM2 <- roc(churn~prob_SM2, data=churn_data_pc_target)
auc(h_SM2)
plot(h_SM2)
```
...
```{r McFadden_SM2}
loadPkg("pscl") # use pR2( ) function to calculate McFadden statistics for model eval
churnLogitpr2_SM2 = pR2(churnLogit_SM2)
churnLogitpr2_SM2
unloadPkg("pscl") 
```

### Customers with a higher salary and account balance are more likely to churn?

```{r logitmodel_SM3,results='markup'}
churnLogit_SM3 <- glm(churn ~ estimated_salary + balance, data = churn_data_pc_target, family = "binomial")
summary(churnLogit_SM3)

xkabledply(churnLogit_SM3, title = paste("Logistic Regression :", format(formula(churnLogit_SM3)) ))
```

```{r ConfInt_SM3, results='markup', collapse=F}
xkabledply( confint(churnLogit_SM3), title = "CIs using profiled log-likelihood" )

xkabledply( confint.default(churnLogit_SM3), title = "CIs using standard errors" )
```

```{r confusionMatrix_SM3, results='markup'}
loadPkg("regclass")

xkabledply( confusion_matrix(churnLogit_SM3), title = "Confusion matrix from Logit Model" )
unloadPkg("regclass")
```

```{r HosmerLemeshow_SM3, results='markup', collapse=F}
loadPkg("ResourceSelection") 
admitLogitHoslem_SM3 = hoslem.test(churn_data_pc_target$churn, fitted(churnLogit_SM3)) 
unloadPkg("ResourceSelection") 

admitLogitHoslem_SM3
```

```{r roc_auc_SM3}
loadPkg("pROC")
prob_SM3 = predict(churnLogit_SM3, type = "response" )
churn_data_pc_target$prob_SM3 = prob_SM3
h_SM3 <- roc(churn~prob_SM3, data=churn_data_pc_target)
auc(h_SM3)
plot(h_SM3)
```
...
```{r McFadden_SM3}
loadPkg("pscl") # use pR2( ) function to calculate McFadden statistics for model eval
churnLogitpr2_SM3 = pR2(churnLogit_SM3)
churnLogitpr2_SM3
unloadPkg("pscl") 
```
...
reg.churn <- regsubsets(churn~.-prob, data = churn_data_pc_target, nvmax =9, nbest = 1, method = "exhaustive")  
...
Using exhaustive search we found that the variables country,gender,age,balance,active_member predict the churn of customer which is a 5 variable best model with adjusted R^2 value 0.14.

The  best model predicted using BIC is gender,age,balance,active_member which is a 4 variable model with BIC value -1400.

The best model predicted using Cp is credit_score,country,gender,age,tenure,balance,active_member which is a 7 variable model. with Cp value 6.8
...
The best model predicted using adjusted R^2 plot is credit_score, country, gender, age, tenure, balance, products_number, active_member, estimated_salary which is a 9 variable model with R^2 value 0.135.

The best model predicted using Mallow Cp plot is crdt_s-cn-g-ag-t-b-p-a_e which is a 9 variable model and crdt_s-cn-g-ag-b-a_ 7 var model.

**Feature selection using forward search**

```{r}
reg.churnforward <- regsubsets(churn~.-prob, data = churn_data_pc_target, nvmax =9, nbest = 1, method = "forward")
...
The best model using forward search is country,gender,age,balance,active_member which is a 5 variable model with adjusted R^2 value of 0.14.

The  best model predicted using BIC is gender,age,balance,active_member which is a 4 variable model with value -1400 Cp.
...
```{r}
reg.churnback <- regsubsets(churn~.-prob, data = churn_data_pc_target, method = "backward", nvmax = 9, nbest = 1)
...
The best model using backward search is country,gender,age,balance,active_member which is a 5 variable model with adjusted R^2 value of 0.14.
...
```{r}
reg.churnseqrep <- regsubsets(churn~.-prob, data =churn_data_pc_target, nvmax = 9, nbest = 1 , method = "seqrep")
...
The best model predicted using Cp is credit_score,country,gender,age,tenure,balance,active_member,estimated_salary which is a 8 variable model with Cp value 7.6
