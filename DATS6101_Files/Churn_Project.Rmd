...
corr5
corr6 <-cor(churn_data_numeric$products_number +churn_data_numeric$active_member, churn_data_numeric$churn)
corr6
```
...
reg.churn <- regsubsets(churn~.-customer_id, data = churn_data_pc_target, nvmax =9, nbest = 1, method = "exhaustive")  
...
```{r}
reg.churnforward <- regsubsets(churn~.-customer_id, data = churn_data_pc_target, nvmax =9, nbest = 1, method = "forward")
...
**Feature selection uisng backaward search**
```{r}
reg.churnback <- regsubsets(churn~.-customer_id, data = churn_data_pc_target, method = "backward", nvmax = 9, nbest = 1)
...
```{r}
reg.churnseqrep <- regsubsets(churn~.-customer_id, data =churn_data_pc_target, nvmax = 9, nbest = 1 , method = "seqrep")
