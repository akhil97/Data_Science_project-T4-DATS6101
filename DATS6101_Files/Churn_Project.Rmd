...
**Checking if there are any NA values in the dataframe**
...
```{r chi_tests_ind}
contable = table(churn_data$churn, churn_data$gender)
xkabledply(contable, title="Contingency table ")
```

```{r, results="markup"}
chitests = chisq.test(contable)
chitests
```
...
```{r}
xkabledply(chitests$expected, title = "Cross table for the expected frequencies between Smoke (habit) vs Exercise (level) in library 'MASS'")
```
...
**We are performing ANOVA test to determine whether churn rate depend on account balance in the customer account.**
```{r}
ggplot(data=churn_data,aes(x=churn,y=balance))+
  geom_boxplot(aes(fill=churn),outlier.color="red",outlier.shape=8,outlier.size=4)+
  labs(title="Account balance vs Churn",x="Customer Churn", y ="Account Balance")
  
```


```{r}
anovaRes = aov(balance ~ churn, data=churn_data)
summary(anovaRes)
```
...
library(corrplot)
churn_data_numeric <- select_if(churn_data, is.numeric)
str(churn_data_numeric)
cor <- cor(churn_data_numeric)
corrplot(cor, method = 'square')
cor_score <- cor(churn_data_numeric$credit_score, churn_data_numeric$churn)
print(cor_score)
```
